...
ENV VERSION=v4.6.1 NPM_VERSION=3
...
RUN apk add --no-cache curl make gcc g++ python linux-headers paxctl libgcc libstdc++ gnupg && \
  ...
  if [ -x /usr/bin/npm ]; then \
    ...
    node-gyp install ${VERSION} && \
    find /usr/lib/node_modules/npm -name test -o -name .bin -type d | xargs rm -rf; \
  fi

ONBUILD ADD . /bundle

ONBUILD RUN cd /bundle && \
...
ONBUILD RUN apk del curl make gcc g++ python linux-headers paxctl gnupg

ONBUILD RUN rm -rf /etc/ssl /node-${VERSION}.tar.gz /SHASUMS256.txt.asc /node-${VERSION} ${RM_DIRS} \
